...
void Test() {
	const double S = 50.0;
	const double K = 55.55;
	const double vol = 0.3;
	const double r = 0.02;
	const double T = 7.0/12.0;
	auto payoff = [=](double spot) { return exp(-r*T) * std::max(0.0, K - spot); };
	auto mc_price = [=](double z) {return S * exp((r - 0.5*pow(vol, 2)) * T + vol*sqrt(T)*z); };
	auto runMC = [payoff, mc_price, r, T](auto normal) {
		auto size = normal.size();
		std::vector<double> put(size);
		std::transform(normal.begin(), normal.begin() + size, put.begin(), [=](double z) {return payoff(mc_price(z)); });
		//std::for_each(normal.begin(), normal.begin() + size, [=](double z) {std::cout << mc_price(z) << "\t" << payoff(mc_price(z))*exp(r*T) << "\t" << exp(-r*T) << 
		//	"\t" << payoff(mc_price(z)) << std::endl; });
		std::cout << std::accumulate(ALL_ELE(put), 0.0) << " " << size << std::endl;
		double sum = 0;
		return std::accumulate(ALL_ELE(put), 0.0) / size;
	};
	std::vector<double> v(50000);
	std::generate(v.begin(), v.end(), getBoxMullerNormalGen());
	std::cout << runMC(v) << std::endl;
}
